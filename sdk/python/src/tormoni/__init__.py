"""The Python SDK for Tormoni.

Tormoni runs untrusted code inside a hardware-isolated virtual machine on your
own machine.  This package spawns nothing and parses nothing: it calls the same
``execute_sandbox`` the ``tormoni`` binary calls, in this process, and the
:class:`Run` it hands back is built in Rust from the run record itself.

    >>> import tormoni
    >>> run = tormoni.Tormoni().run(["python3", "-c", "print(6*7)"])
    >>> run.stdout
    '42\\n'
"""

from .client import Sandbox, Tormoni
from .exceptions import TormoniError, TormoniException, TormoniNotFound
from .models import File, Posture, Run
from .options import Pairs, RunOptions

__version__ = "0.1.0"

__all__ = [
    "Tormoni",
    "Sandbox",
    "Run",
    "RunOptions",
    "Pairs",
    "Posture",
    "File",
    "TormoniException",
    "TormoniError",
    "TormoniNotFound",
    "__version__",
]
