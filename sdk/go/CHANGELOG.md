...
- `Run`, `Posture`, `Mount`, `Share` and `File`, with explicit JSON tags for every field, plus
  `Run.OK`.
- `RunOptions`, mapping one-to-one onto the flags `boxdesk run` accepts.
- `ErrNotFound` and `*Error`, which carries Boxdesk's stderr verbatim, its exit code, and the
...
Long-lived sandboxes (`boxdesk up`) are not covered.
