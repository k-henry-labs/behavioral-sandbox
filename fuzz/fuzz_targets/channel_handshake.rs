...
//! parity so every exposed `tormoni_channel::fuzz::decode_*` entry point has a deep target.
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    tormoni_channel::fuzz::decode_handshake(data);
    // Correct magic, fuzzed version bytes: the version check and the accept path.
    tormoni_channel::fuzz::decode_handshake_after_magic(data);
});
