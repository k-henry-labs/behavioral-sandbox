...
//!
//! **This crate may use `unsafe`**, because libkrun is a C library; `boxdesk-app`'s window chrome is
...
//! libkrun, and what stops a running VM is a signal to the helper process (`boxdesk-supervisor`'s
        ...
        match self {
            ...
            Self::NotLinked { call } => write!(
                ...
                "{call} cannot be called: this build links no libkrun (install libkrun, or set \
                 BOXDESK_KRUN_LIB_DIR, and rebuild)"
            ),
...
/// can only come from whatever spawned it. `boxdesk-supervisor` puts it on the helper it spawns.
///
/// `None` on Linux, whose loader finds `libkrunfw.so` beside libkrun without help.
pub const KRUNFW_DIR: Option<&str> = option_env!("BOXDESK_KRUNFW_DIR");
...
mod tests {
    ...
    fn a_build_without_libkrun_reports_the_library_as_missing() {
        ...
        assert!(msg.contains("BOXDESK_KRUN_LIB_DIR"), "names the fix: {msg}");
    }
    ...
    fn an_input_device_answers_the_probe_queries_through_its_table() {
        let device = Arc::new(
            InputDevice::new("boxdesk test device")
                .serial("SERIAL-1")
        ...
        assert_eq!((got, &name[..]), (4, &b"boxd"[..]), "cut to the buffer");
