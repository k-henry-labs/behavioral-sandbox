...
//! for by name, and the directory is baked in for `boxdesk-supervisor` to put on the helper's
...
fn main() {
    ...
    println!("cargo::rerun-if-env-changed=BOXDESK_KRUN_LIB_DIR");
    println!("cargo::rerun-if-env-changed=BOXDESK_KRUNFW_LIB_DIR");

    if let Some(dir) = std::env::var_os("BOXDESK_KRUN_LIB_DIR") {
        ...
        .output()
    {
        ...
        _ => println!(
            "cargo::warning=libkrun not found (no pkg-config entry). boxdesk-krun compiled its \
             declarations, but nothing can link them: install libkrun, or set BOXDESK_KRUN_LIB_DIR."
        ),
    }
}
...
fn emit_krunfw_dir(near: &[PathBuf]) {
    ...
    if let Some(dir) = std::env::var_os("BOXDESK_KRUNFW_LIB_DIR") {
        println!(
            "cargo::rustc-env=BOXDESK_KRUNFW_DIR={}",
            dir.to_string_lossy()
        );
        return;
    }
    match near.iter().find_map(|dir| find_krunfw(dir)) {
        Some(found) => println!("cargo::rustc-env=BOXDESK_KRUNFW_DIR={}", found.display()),
        None => println!(
            "cargo::warning=libkrunfw not found near libkrun. A sandbox will fail to boot with \
             \"Couldn't find or load libkrunfw\": set BOXDESK_KRUNFW_LIB_DIR to the directory holding \
             it."
        ),
    }
}
