...
//! reports and the line grammar are `boxdesk_input`'s; this is what the helper adds to them.
//!
//! - **`BOXDESK_INPUT_REPLAY`** names a file or FIFO of lines the helper feeds to the devices as a
...
use boxdesk_input::{Button, Target, feed};
use boxdesk_krun::{InputEvent, InputSender};
use winit::event::MouseButton;

/// The environment variable naming the replay source.
pub(crate) const REPLAY_ENV: &str = "BOXDESK_INPUT_REPLAY";
...
pub(crate) fn replay(path: PathBuf, inputs: Inputs) -> io::Result<()> {
    std::thread::Builder::new()
        .name("boxdesk-input-replay".to_string())
        .spawn(move || {
            let Ok(file) = std::fs::File::open(&path) else {
                eprintln!(
                    "boxdesk __vmm: warning: {REPLAY_ENV}={} could not be opened; no input replayed",
                    path.display()
                );
                return;
            };
            feed(
                ...
                |line| eprintln!("boxdesk __vmm: warning: replay line ignored: {line:?}"),
            );
        })
        .map(drop)
}
...
pub(crate) fn serve(stream: UnixStream, inputs: Inputs) -> io::Result<()> {
    std::thread::Builder::new()
        .name("boxdesk-input-session".to_string())
        .spawn(move || {
            feed(
                BufReader::new(stream),
                |target, events| inputs.send(target, events),
                |line| eprintln!("boxdesk __vmm: warning: input line ignored: {line:?}"),
            );
        })
        .map(drop)
}
...
mod tests {
    ...
    fn winit_buttons_map_to_the_devices_codes() {
        ...
        assert_eq!(
            ...
            Some(boxdesk_input::BTN_RIGHT)
        );
        assert_eq!(
            button_code(MouseButton::Middle),
            Some(boxdesk_input::BTN_MIDDLE)
        );
        assert_eq!(
            button_code(MouseButton::Back),
            Some(boxdesk_input::BTN_SIDE)
        );
        assert_eq!(
