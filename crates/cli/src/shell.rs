//! `boxdesk shell`: an interactive shell (or any command) on a pseudo-terminal inside a fresh sandbox.
//!
...
use boxdesk_supervisor::{Console, Vm, VmConfig};
...
fn session(args: &ShellArgs) -> Result<u8, String> {
    ...
    if results {
        cfg.mounts.push((
            PathBuf::from(boxdesk_record::RESULTS_GUEST_PATH),
            run.results(),
        ));
    }
    ...
    record.finish(match &outcome {
        Ok(code) => boxdesk_record::End::Exit(i32::from(*code)),
        Err(_) => boxdesk_record::End::Failed,
    });
...
fn attach(
    ...
    name: String,
    command: Vec<String>,
    log: &mut boxdesk_record::Capped,
    record: &mut boxdesk_record::Record,
    store: &boxdesk_record::Store,
) -> Result<u8, String> {
...
impl SessionDir {
    fn create() -> Result<Self, String> {
        ...
        let path = std::env::temp_dir().join(format!("boxdesk-shell-{}", std::process::id()));
