//! End-to-end tests for the user-facing verbs: each boots a real guest through the built `boxdesk`.
//!
...
//! a skipped test as a pass. Run with `cargo test -p boxdesk --test e2e -- --ignored`.
...
/// The `boxdesk` under test: the binary cargo built for this test run, never a `PATH` lookup.
fn boxdesk() -> Command {
    ...
    cmd.env("BOXDESK_RUNS_DIR", runs_dir());
    cmd
}

/// A runs directory for this test process alone.
fn runs_dir() -> PathBuf {
    std::env::temp_dir().join(format!("boxdesk-e2e-runs-{}", std::process::id()))
}
...
/// The guest command's exit code is the verb's exit code, which is what lets `boxdesk run` sit in a
...
/// The environment layer: with no flag, `BOXDESK_VCPUS` decides, which is the flag-then-env order
...
fn a_mounted_directory_is_read_write_and_edits_land_on_the_host() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-mount");
...
fn a_second_virtiofs_tag_carries_a_directory_the_guest_mounts_itself() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-share");
...
fn the_default_guest_cannot_write_the_image_it_boots_from() {
    ...
    let probe = guest_root().join("boxdesk-write-probe");
    ...
    let write = |posture: &str| -> String {
        let out = boxdesk()
            ...
            .args([
                ...
                "sh",
                "-c",
                "echo guest > /boxdesk-write-probe && echo WROTE || echo blocked",
            ])
            .output()
            .expect("run boxdesk");
        String::from_utf8_lossy(&out.stdout).trim().to_string()
...
fn a_dry_run_prints_the_posture_and_boots_nothing() {
    let runtime = boxdesk_test_support::ScratchDir::created("e2e-dry-run");
    ...
    let socket = runtime.path().join("boxdesk").join(format!("{name}.sock"));
...
fn a_share_tag_that_would_shadow_a_mount_is_refused_before_boot() {
    ...
    assert_eq!(out.status.code(), Some(2));
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(err.contains("boxdesk-mnt-0"), "names the tag: {err}");
...
fn a_frame_the_guest_draws_reaches_the_host() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-frame");
...
fn a_display_guest_is_offered_a_3d_virtio_gpu_it_has_no_driver_for() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-gpu");
...
fn a_synthetic_key_and_click_reach_a_guest_process() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-input");
    ...
    let mount = format!("/mnt={}", dir.path().display());
    let mut child = boxdesk()
        .arg("run")
        .arg("--root")
        .arg(guest_root())
        .args(["--display", "320x240", "--mount", &mount])
        .env("BOXDESK_INPUT_REPLAY", &fifo)
        .env_remove("DISPLAY")
        .env_remove("WAYLAND_DISPLAY")
    ...
    let status = child.wait().expect("wait for boxdesk");
    let out = lines.concat() + &rest;
    let err = stderr.join().unwrap_or_default();
    assert!(status.success(), "stdout: {out}\nstderr: {err}");
    assert!(
        out.contains("boxdesk keyboard") && out.contains("boxdesk pointer"),
...
fn the_desktop_image_boots_to_a_session_the_keyboard_reaches() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-desktop");
    ...
    while std::time::Instant::now() < deadline {
        keys.write_all(typed("boxdesk-typed-this\n").as_bytes())
...
fn a_frame_log_records_each_frame_the_guest_flushed() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-frame-log");
    std::fs::write(dir.path().join("draw.py"), include_str!("drm_draw.py"))
        .expect("stage the drawer");
...
fn a_second_process_maps_the_frames_the_guest_draws() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-boundary");
...
fn run_refuses_a_missing_root_before_booting() {
    let out = boxdesk()
        .args(["run", "--root", "/nonexistent-boxdesk-root", "--", "true"])
        .output()
        .expect("run boxdesk");
    assert_eq!(out.status.code(), Some(2));
    let err = String::from_utf8_lossy(&out.stderr);
...
fn a_key_and_click_over_the_control_socket_reach_a_guest_process() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-wire-input");
    let rt = dir.path().join("rt");
    std::fs::create_dir(&rt).expect("a runtime dir");
    std::fs::set_permissions(&rt, std::fs::Permissions::from_mode(0o700)).expect("private");
    let mount_dir = dir.path().join("m");
    ...
    let socket = boxdesk_supervisor::socket::path_in(&rt, "wired").expect("a socket path");
    let mut session = boxdesk_supervisor::control::input(&socket).expect("an input session");
    for line in [
        "kbd 1 30 1",
        "kbd 0 0 0",
        "kbd 1 30 0",
...
/// `/results` in the record's own directory; `boxdesk show` reads it back and `boxdesk rm` removes it.
#[test]
#[ignore = "boots a real guest: needs /dev/kvm and the guest tree"]
fn a_run_leaves_its_record_output_and_results() {
    ...
    assert_eq!(record.verb, boxdesk_record::Verb::Run);
    assert_eq!(record.end, Some(boxdesk_record::End::Exit(3)));
    assert!(record.posture.results, "results on by default");
    assert_eq!(record.posture.network, boxdesk_record::Network::None);
...
fn an_up_sandbox_records_its_execs_and_its_stop() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-up-record");
    let rt = dir.path().join("rt");
    std::fs::create_dir(&rt).expect("a runtime dir");
    std::fs::set_permissions(&rt, std::fs::Permissions::from_mode(0o700)).expect("private");
...
fn a_guest_that_sets_a_new_mode_is_followed_by_the_next_lease() {
    ...
    let dir = boxdesk_test_support::ScratchDir::created("e2e-mode");
    let rt = dir.path().join("rt");
    std::fs::create_dir(&rt).expect("a runtime dir");
    std::fs::set_permissions(&rt, std::fs::Permissions::from_mode(0o700)).expect("private");
    let mount_dir = dir.path().join("m");
