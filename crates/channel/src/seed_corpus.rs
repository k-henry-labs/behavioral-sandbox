...
//!   `BOXDESK_UPDATE_FUZZ_SEEDS`; otherwise it compares, and an orphaned directory is an extra key.
...
const UPDATE: &str = "BOXDESK_UPDATE_FUZZ_SEEDS";
...
fn the_committed_seeds_are_what_the_encoders_produce() {
    ...
    let fix = format!("re-generate them with `{UPDATE}=1 cargo test -p boxdesk-channel`");
