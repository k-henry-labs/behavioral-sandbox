...
//! any more surface.
//!
//! These spawn real guests, so they need `/dev/kvm`, the guest tree and a built `boxdesk`. Each
...
//!
//! Run them with `cargo test -p boxdesk-supervisor --test leak -- --ignored --test-threads=1`. Serial,
...
fn skip_reason() -> Option<String> {
    ...
    if !boxdesk_binary().is_file() {
        return Some(format!(
            "no boxdesk binary at {} (run `cargo build -p boxdesk`)",
            boxdesk_binary().display()
        ));
    }
    None
}
...
fn boxdesk_binary() -> PathBuf {
    workspace_root().join("target/debug/boxdesk")
}
...
fn runtime_dir(tag: &str) -> boxdesk_test_support::ScratchDir {
...
#[test]
#[ignore = "boots a real guest: needs /dev/kvm, the guest tree and a built boxdesk"]
fn a_supervisor_killed_mid_boot_leaves_no_vm() {
    ...
    let sock = runtime.path().join("boxdesk/midboot.sock");
    ...
    let scan_dir = runtime.path().join("boxdesk");
    assert!(
        boxdesk_supervisor::discover::live_in(&scan_dir)
    ...
    assert!(
        !boxdesk_supervisor::socket::is_live(&sock),
...
/// Dropping a [`boxdesk_supervisor::Vm`] must take the VM with it, including when the guest is a live
...
fn dropping_the_supervisor_takes_the_running_vm_with_it() {
    ...
    let sock = runtime.path().join("boxdesk/dropme.sock");
...
fn a_killed_vms_socket_is_left_but_never_counted_as_live() {
    ...
    let sock = runtime.path().join("boxdesk/stale.sock");
    ...
    assert!(eventually(|| !boxdesk_supervisor::socket::is_live(&sock)));
    ...
    assert!(
        boxdesk_supervisor::socket::clear_if_stale(&sock).expect("clear the leftover"),
