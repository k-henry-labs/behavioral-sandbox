...
//! - **The helper is reached through `current_exe()`, never `PATH`.** Spawning `boxdesk` by name would
...
const HELPER_MARKER: &str = "BOXDESK_VMM";
...
#[non_exhaustive]
pub enum Console {
    /// The caller's own stdin and stdout, which is what makes `boxdesk run` behave like the command
    /// it wraps.
    #[default]
    Inherited,
    ...
    /// drains through [`Vm::take_stdout`] and [`Vm::take_stderr`]: how `boxdesk run` copies the
...
pub struct Vm {
    ...
    child: Option<Child>,
    /// The name a caller gave this VM. The helper binds `<runtime>/boxdesk/<name>.sock` with it, which
...
fn kernel_payload_path() -> Option<OsString> {
    let dir = boxdesk_krun::KRUNFW_DIR?;
...
pub mod socket {
    ...
    /// create `boxdesk/` first. Checked on every resolution, since the directory outlives its maker.
    ///
...
pub mod control {
    ...
    impl InputSession {
        /// Sends one `kbd|ptr TYPE CODE VALUE` line. The grammar is `boxdesk-input`'s; this carries
        /// the text.
...
pub mod discover {
    ...
    /// a test giving its VMs a private `XDG_RUNTIME_DIR` scans `<that>/boxdesk`, where the real one
...
mod tests {
    ...
    /// same property a shipped `boxdesk` relies on: re-execute *me*, not whatever `PATH` says.
    #[test]
    ...
    /// The program is **this executable**, through `current_exe`, not "boxdesk" on a `PATH` search.
    ...
    fn the_helper_is_told_where_the_kernel_payload_is() {
        let Some(dir) = boxdesk_krun::KRUNFW_DIR else {
            eprintln!(
                "skipped: no libkrunfw was found when boxdesk-krun was built, so there is no \
...
mod socket_tests {
    ...
    fn a_runtime_directory_that_is_a_symlink_is_refused() {
        use std::os::unix::fs::DirBuilderExt;
        let dir = boxdesk_test_support::ScratchDir::created("runtime-symlink");
    ...
    fn a_listening_socket_is_live_and_a_leftover_file_is_not() {
        let dir = boxdesk_test_support::ScratchDir::created("sock-live");
    ...
    fn clearing_an_absent_socket_is_not_an_error() {
        let dir = boxdesk_test_support::ScratchDir::created("sock-absent");
...
mod control_tests {
    ...
    fn a_display_answer_carries_the_layout_the_fd_and_then_records() {
        ...
        let dir = boxdesk_test_support::ScratchDir::created("display-lease");
    ...
    fn waiting_on_a_lease_says_whether_a_record_is_there() {
        let (server, client) = std::os::unix::net::UnixStream::pair().expect("a socket pair");
        let dir = boxdesk_test_support::ScratchDir::created("display-wait");
        let file = std::fs::File::create(dir.path().join("frames")).expect("a file to hand over");
        let scanout = control::Scanout::new(320, 240, 2, 1280, 4, 307_200, 3);
        let whole = control::Damage::new(0, 0, 320, 240);
...
mod discover_tests {
    ...
    fn a_scan_reports_listeners_and_skips_everything_else() {
        let dir = boxdesk_test_support::ScratchDir::created("discover");
        let d = dir.path();
    ...
    fn the_agent_channel_sits_beside_the_control_socket_and_is_not_a_vm() {
        ...
        let dir = boxdesk_test_support::ScratchDir::created("agent-socket-scan");
        let listener =
    ...
    fn reaping_a_dead_vm_takes_its_agent_channel_too() {
        let dir = boxdesk_test_support::ScratchDir::created("agent-socket-reap");
    ...
    fn a_scan_does_not_remove_leftovers_but_reaping_does() {
        let dir = boxdesk_test_support::ScratchDir::created("discover-reap");
        let d = dir.path();
