...
mod tests {
    ...
    /// **No commit on this branch credits a coding agent.**
    ///
    /// A `Co-Authored-By:` trailer is not a courtesy line: GitHub reads it as a second author, so
    /// one trailer puts that account in the repository's contributor graph and in the
    /// `git shortlog` of every release. Taking it back out means rewriting published history and
    /// force-pushing over whatever anyone else has pulled. It happened here, which is why the
    /// rule now has a test instead of only a sentence in `AGENTS.md`.
    ///
    /// Matched on the credit FORMS — a trailer, a "generated with" line, the robot emoji, an
    /// author or committer identity — and not on the bare word, because `CLAUDE.md` is a filename
    /// this repo's log is allowed to mention and a text `Cursor` is a type in it. Dependabot is
    /// not covered: it opens its own pull requests under its own name, and rewriting those commits
    /// would rewrite nearly every merge in the history.
    #[test]
    fn the_log_credits_no_agent() {
        // The shapes a tool reaches for when it signs work that is not its own.
        const AGENTS: [&str; 12] = [
            "claude",
            "anthropic",
            "copilot",
        ...
        let log = std::process::Command::new("git")
            ...
            .args([
                "log",
                "--format=%H%x00%an <%ae>%x00%cn <%ce>%x00%B%x01",
                "-n",
                "400",
            ])
            .output();
        let Ok(log) = log.map(|out| out.status.success().then_some(out)) else {
            println!("skipped: git could not be run here, so no log was checked");
            return;
        };
        let Some(log) = log else {
            // A tarball build, or a machine with no git. Say so: a skipped test is not a pass.
            println!("skipped: no readable git history here, so no log was checked");
            return;
        };
        ...
        for commit in text.split('\u{1}').filter(|c| !c.trim().is_empty()) {
            let mut parts = commit.trim_start_matches('\n').split('\u{0}');
            let (Some(sha), Some(author), Some(committer), Some(body)) =
            ...
            let named = |s: &str| {
                ...
                AGENTS.iter().any(|a| lower.contains(a))
            };
            for who in [author, committer] {
                if named(who) {
                    credited.push(format!("{short} is authored by {who}"));
                }
            }
            for line in body.lines() {
                let lower = line.trim().to_lowercase();
                let credit = CREDITS.iter().any(|c| lower.starts_with(c))
                    || lower.contains("generated with")
                    || lower.contains("generated by");
                if (credit && named(&lower)) || line.contains('\u{1F916}') {
                    credited.push(format!("{short} says {}", line.trim()));
                }
            }
        }
        assert!(commits > 0, "no commits read, so this lint proves nothing");
        assert!(
            credited.is_empty(),
            "a commit credits a coding agent, which GitHub counts as a contributor:\n  {}\n\
             The work belongs to the person who ran the tool. Amend the message.",
            credited.join("\n  ")
        );
    }
